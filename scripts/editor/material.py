...
class Material:
    ...
    def _create_unlit_material(self) -> None:
        ...
        if self.basecolor_tex_path:
            ...
            # set color
            self._logger.debug(f'No base color texture. Set self.multiply_color: {self.multiply_color}.')
            color_node = node_tree.nodes.new(type='ShaderNodeEmission')
            color_node.inputs[0].default_value = self.multiply_color
    ...
    def _create_standard_material(self) -> None:
        ...
        if self.basecolor_tex_path:
            ...
            shader_node.inputs[0].default_value = self.multiply_color
...
def create_and_add_material(object: bpy.types.Object, name: str) -> bpy.types.Material:
    new_material = bpy.data.materials.new(name)
    if object.data.materials:
        object.data.materials[0] = new_material
    else:
